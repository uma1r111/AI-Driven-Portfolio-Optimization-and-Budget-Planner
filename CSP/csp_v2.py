...
#############################################
# BudgetCSP Class (Discrete Optimization with Advanced Risk Constraint)
#############################################
class BudgetCSP:
    ...
    def define_constraints(self):
        ...
        def risk_constraint(assignment):
            ...
            if total_alloc == 0:
                return True
            # Compute weights from allocated dollars.
            weights = []
            for stock in self.curated_options:
        ...
        print("\nConstraints defined: Total investment must be <= available budget "
              f"(${self._available_budget}) and portfolio variance must be below "
              f"{risk_thresholds[self.risk_tolerance]} (risk tolerance: {self.risk_tolerance}).")
