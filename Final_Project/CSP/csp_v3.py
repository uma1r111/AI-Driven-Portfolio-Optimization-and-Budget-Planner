...
class BudgetCSP:
    ...
    def define_constraints(self):
        ...
        def semi_hard_diversification_constraint(assignment):
            total_alloc = sum(assignment.values())
            if total_alloc == 0:
                return False
            nonzero_allocs = [v for v in assignment.values() if v > 0]
            nonzero_allocs.sort(reverse=True)
            top_k = nonzero_allocs[:self.min_stocks]
            top_k_total = sum(top_k)
            return (top_k_total / total_alloc) >= 0.75  # at least 75% of total alloc must be in top-k stocks
        ...
        self.constraints.append(semi_hard_diversification_constraint)
        ...
        print(f"  - Top {self.min_stocks} stocks must comprise at least 75% of allocation")
